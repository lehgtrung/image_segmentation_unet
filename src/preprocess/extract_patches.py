...
def get_data_testing_overlap(DRIVE_test_imgs_original, DRIVE_test_groudTruth,
                             Imgs_to_test0, Imgs_to_test1, patch_height, patch_width, stride_height, stride_width):
    # test
    test_imgs_original = load_hdf5(DRIVE_test_imgs_original)
    test_masks = load_hdf5(DRIVE_test_groudTruth)
    ...
    test_masks = test_masks/255.
    # extend both images and masks so they can be divided exactly by the patches dimensions
    test_imgs = test_imgs[Imgs_to_test0:Imgs_to_test1, :, :, :]
    test_masks = test_masks[Imgs_to_test0:Imgs_to_test1, :, :, :]
    x = np.reshape(test_imgs, (test_imgs.shape[2], test_imgs.shape[3]))
    x = Image.fromarray((255*x).astype(np.uint8))  # for 1 image
    x.save('test_img.png')
    x = np.reshape(test_masks, (test_imgs.shape[2], test_imgs.shape[3]))
    x = Image.fromarray((255 * x).astype(np.uint8))  # for 1 image
    x.save('mask_img.png')
...
def is_patch_inside_FOV(x, y, img_w, img_h, patch_h):
    ...
    # radius is 270 (from DRIVE db docs), minus the patch diagonal (assumed it is a square
...
def extract_ordered(full_imgs, patch_h, patch_w):
    ...
    if img_h%patch_h != 0:
        print("warning: " + str(N_patches_h) +
              " patches in height, with about " + str(img_h%patch_h) + " pixels left over")
    N_patches_w = int(img_w/patch_w)  # round to lowest int
    if img_h%patch_h != 0:
        print("warning: " + str(N_patches_w) +
              " patches in width, with about " + str(img_w%patch_w) + " pixels left over")
...
def paint_border_overlap(full_imgs, patch_h, patch_w, stride_h, stride_w):
    ...
    if leftover_w != 0:   # change dimension of img_w
        ...
        tmp_full_imgs = np.zeros((full_imgs.shape[0], full_imgs.shape[1],
                                  full_imgs.shape[2], img_w+(stride_w - leftover_w)))
